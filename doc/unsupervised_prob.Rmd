...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r unigramseries, fig.caption = "The evolution of per-microbe posterior means over time. The first and second antibiotics time courses are introduced at days 12 and 44, respectively."}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r unigramboxplots, fig.width = 10.5, fig.height = 8, echo = FALSE, fig.caption = "The evolution of per-microbe posterior quantiles over time, as different antibiotics time courses are introduced."}
```
\endgroup
\elandscape
...
Paralleling our experiments above, we again clear our code environment and
prepare it for a new analysis.
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualize-theta, fig.width = 6, eval = TRUE}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualize-beta, fig.width = 10, fig.height = 6, eval = TRUE, echo = FALSE}
```
\endgroup
\elandscape
