...
read_chunk("../src/sim/nmf/nmf_visf.R")
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizethetasfigure, eval = TRUE, out.height = 4, fig.cap = "Visualized scores from the NMF experiment, on data without any zero inflation."}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizebetasfigure, eval = TRUE, out.height = 4, fig.cap = "Posteriors for the two-dimensional latent factors, across simulation settings."}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizezinfthetasfigure, eval = TRUE, out.height = 8, out.width = 8, fig.cap = "Each panel displays the estimated scores for a different simulation setting, with a focus on the effect of zero inflation. Columns distinguish between different zero-inflation probabilities and modeling techniques."}
include_graphics("figure/visualizezinfthetasfigure-1.pdf")
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizezinfthetashistfigure, eval = TRUE, fig.cap = "For each simulation setting, we can compute the true estimation errors across samples."}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizezinfbetasfigure, eval = TRUE, out.height = "6in", out.width = "6in", fig.cap = "The analogous display for estimated latent factors, across zero-inflation regimes."}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizezinfbetashist, eval = TRUE, fig.cap = "Across settings, we can explicitly compute estimation errors."}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r betacontours1, out.width = 7, out.height = 4, fig.cap = "True and fitted parameters $K = 2$ topic distributions across $V = 10 features. Different columns describe different $\\left(D, N\\right)$ combinations, while rows give alternative fitting approaches."}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r betacontours2, out.width = 7, out.height = 4, fig.cap = "The analogous figure, with distributions over $V = 50$ instead of 10 features."}
```
\endgroup
...
"10in"$, because the absolute sizes of the clouds in Figure \@ref(fig:betacontours2)
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r betahistograms, out.height = "6in", fig.cap = "Histograms of the difference between sampled topic parameters and their true values, across experimental configurations."}
```
\endgroup
...
\blandscape
```{r betaboxplot1, out.width = "10in", out.height = "6in", fig.cap = "Boxplots of posterior samples for the $\\beta_{\\cdot k}$ across features, inferential techniques, and experimental configurations, when $V = 10$.", echo = FALSE}
```
\elandscape

\blandscape
```{r betaboxplot2, out.width = "10in", out.height = "6in", fig.cap = "Boxplots of posterior samles, when $V = 50$.", echo = FALSE}
```
\elandscape
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r thetaboxplot1, out.width = "10in", out.height = "6in", echo = FALSE, fig.cap = "Estimated and true proportions $\\theta_{i1}$ across samples, for simulations where $D = 20$."}
```
\endgroup
\elandscape
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r thetaboxplot2, out.width = "10in", out.height = "6in", echo = FALSE, fig.cap = "The analogous figure, when $D = 100"}
```
\endgroup
\elandscape
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r histogramsfigure, fig.cap = "Even after filtering, the data are very heavy tailed. Upon asinhl ransformation, the data are more regularly shaped, though still somewhat heavy tailed.", eval = TRUE}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r heatmapsfigure, fig.show = "hold", fig.align = "left", fig.cap = "Heatmaps of counts on the filtered data, before (left) and after (right) transformation, with microbes and samples sorted according to their average value.", eval = TRUE, out.height = "1.5in"}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizethetaboxplotfigure, out.width = "10in", out.height = "6in", fig.cap = "Boxplots of approximate posteriors for estimated mixture memberships, and their evolution over time.", echo = FALSE, eval = TRUE}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizebetafigure, out.width = "10in", out.height = "6in", fig.cap = "The distribution over microbes associated with each LDA topic.", echo = FALSE, eval = TRUE}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r unigramseriesfigure, fig.cap = "The evolution of per-microbe posterior means over time. The first and second antibiotics time courses are introduced at days 12 and 44, respectively.", eval = TRUE}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r unigramboxplotsfigure, out.width = "10in", out.height = "6in", echo = FALSE, fig.cap = "The evolution of per-microbe posterior quantiles over time, as different antibiotics time courses are introduced.", eval = TRUE}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizealphafigure, eval = TRUE}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizemufigure, out.width = "10in", out.height = "6in", echo = FALSE, eval = TRUE}
