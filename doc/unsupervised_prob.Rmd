...
The code below retrieves the output from the simulation experiments.
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r betacontours2, fig.width = 7, fig.height = 4, eval = TRUE, fig.cap = "The analogous figure , with distributions over $V = 50$ instead of 10 features."}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r betahistograms, eval = TRUE, fig.height = 6, fig.cap = "Histograms of the difference between sampled topic parameters and their true values, across experimental configurations."}
```
\endgroup
...
\blandscape
```{r betaboxplot1, fig.width = 10, fig.height = 6, eval = TRUE, fig.cap = "Boxplots of posterior samples for the $\\beta_{\\cdot k}$ across features, inferential techniques, and experimental configurations, when $V = 10$."}
```
\elandscape

\blandscape
```{r betaboxplot2, fig.width = 10, fig.height = 6, eval = TRUE, fig.cap = "Boxplots of posterior samles, when $V = 50$."}
```
\elandscape
...
proportions $\left(\theta_{i}\right)$.

\begingroup
\setstretch{1}
\fontsize{8pt}{8pt}
```{r theta-samples, eval = TRUE}
```
\endgroup
...
antibiotic treatment. Here, several patients were sampled for several weeks 
