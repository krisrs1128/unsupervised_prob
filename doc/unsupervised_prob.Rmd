...
topics, we align those with the next highest correlation, and so on.}. Figure
\@ref(betacountours1) displays the true and posterior $\beta_{k}$ for the
...
Interestingly, when $D$ and $N$ are small ($=20$), VB appears more accurate than
either the MCMC or bootstrappign apparoches, whose posterior means all lie along
the diagonal, corresponding to topics that both reflect the global marginal
feature counts across samples. This effect is especially pronounced in the case
$V = 50$. Here, even the $D = 20, N = 50$ configuration seems to lead to result
in inference issues for the bootstrap.
