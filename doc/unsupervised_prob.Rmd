...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r heatmaps, eval = TRUE, fig.show = "hold"}
```
\endgroup
...
VB, because MCMC seemed to produce degenerate fits\footnote{Many of the
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r extract_theta, echo = FALSE, eval = TRUE}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualize_theta_heatmap, eval = TRUE}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualize_theta_boxplot, eval = TRUE}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualize_beta, eval = TRUE, fig.width = 10, fig.height = 6, echo = FALSE}
```
\endgroup
\elandscape
...
relatively more concentrated on a small subset of microbes reflects a decrease
in community diversity during antibiotic time courses. It is interesting that
the two time courses can be identified with different microbial signatures.
