...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizethetas-prep, eval = TRUE}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizethetas, fig.height = 6, fig.cap = "Visualized scores from the NMF experiment, on data without any zero inflation.", eval = FALSE}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizethetashist, eval = FALSE, fig.cap = "Histograms of the differences between posterior sampled scores and their true values."}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizebetas, eval = FALSE, fig.height = 6, fig.cap = "Posteriors for the two-dimensional latent factors, across simulation settings."}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizebetashist, eval = FALSE, fig.cap = "Histograms of the differences between posterior sampled factors and their true values."}
```
\endgroup
...
* In the case that it is known that zero-inflation is present and the
  appropriate model, with known zero inflation probability, is used, how quickly
  do estimates deterioriate as more and more entries are sent to zero?
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizezinfthetas, fig.height = 8, fig.width = 8}
```
\endgroup
