...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r histograms, fig.cap = "Even after filtering, the data are very heavy tailed. Upon $\asinh$ transformation, the data are more regularly shaped, though still somewhat heavy tailed."}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r heatmaps, fig.show = "hold", fig.cap = "Heatmaps of counts on the filtered data, before (left) and after (right) transformation, with microbes and samples sorted according to their average value."}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualize_theta_heatmap}
```
\endgroup

A disadvantage of Figure \@ref(fig:visualize_theta_heatmap) is that it displays
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualize_beta, fig.width = 10, fig.height = 6, echo = FALSE}
```
\endgroup
\elandscape
...
explicitly setting $\sigma$. Instead, we use a hyperprior, $\sigma \sim \InvGam
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r unigramseries, eval = TRUE, fig.caption = "The evolution of per-microbe posterior means over time. The first and second antibiotics time courses are introduced at days 12 and 44, respectively."}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r unigramboxplots, eval = TRUE, fig.width = 10.5, fig.height = 8, echo = FALSE, fig.caption = "The evolution of per-microbe posterior quantiles over time, as different antibiotics time courses are introduced."}
```
\endgroup
\elandscape
...
probabilities\footnote{These are in fact a column of the \texttt{beta_hat}
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualize-beta, fig.width = 10, fig.height = 6}
```
\endgroup
\elandscape
