...
\@ref(visthetashist). Each panel has the same meaning as before. Each histogram
are of errors $\hat{\theta}_{ik} - \theta_{ik}$; orange is for $k = 1$ and
purple is for $k = 2$, though the error distributions for the two factor
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizethetashist, fig.cap = "Visualized scores from the NMF experiment, on data without any zero inflation."}
```
\endgroup
