...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizethetas, fig.height = 6, fig.cap = "Visualized scores from the NMF experiment, on data without any zero inflation."}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizebetas, fig.height = 6, fig.cap = "Posteriors for the two-dimensional latent factors, across simulation settings."}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r visualizezinfbetas, fig.height = 8, fig.width = 8, fig.cap = "The analogous display for estimated latent factors, across zero-inflation regimes."}
```
\endgroup
...
through \@ref(betahistograms). Figure \@ref(betacountours1) displays the true
and posterior $\beta_{k}$ for the experiments with $V = 10$ features, while
other simulation characteristics are varied. As before, each panel represents a
single experimental configuration, with two axes associated with the two
underlying topics. Each black number $v$ is the true value of feature $v$ across
topics: $\left(\beta_{v1}, \beta_{2}\right)$. The shaded clouds are sampled
posteriors, while the orange labels give posterior means.
...
total number of documents, and $N$, the number of words in each document. Figure
\@ref(fig:betacontours2) is read in exactly the same way.
...
with $D$, begins to concentrate around its true value. When $V = 50$, this
concentration is somewhat slower than when $V = 10$; however, the larger
absolute sizes of the clouds in Figure \@ref(fig:betacontours2) is also a result
of their being more parameters.
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r libraries-boot-expers, eval = TRUE}
```
\endgroup
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r beta-alignment, eval = TRUE}
```
\endgroup

```{r beta-contours-object, eval = TRUE}
```

```{r betacontours1, fig.width = 7, fig.height = 4, eval = TRUE, fig.cap = "True and fitted parameters $K = 2$ topic distributions across $V = 10$ features. Different columns describe different $\\left(D, N\\right)$ combinations, while rows give alternative fitting approaches."}
```

```{r betacontours2, fig.width = 7, fig.height = 4, eval = TRUE, fig.cap = "The analog of Figure \\@ref(fig:betacontours1), with distributions over $V = 50$ instead of 10 features."}
```

```{r beta-boxplots-object, eval = TRUE}
```
\blandscape
```{r betaboxplot1, fig.width = 10, fig.height = 4.5, eval = TRUE}
```
\elandscape

\blandscape
```{r betaboxplot2, fig.width = 10, fig.height = 4.5, eval = TRUE}
```
\elandscape
...
\setstretch{1}
\fontsize{8pt}{8pt}
```{r betahistograms, eval = TRUE, fig.height = 6}
```
\endgroup
