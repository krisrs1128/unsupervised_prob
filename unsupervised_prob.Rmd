...
is, suppose $n_{d\left(t\right) \cdot}$ is a document at time $t$. Then, we
imagine $n_{d\left(t\right)} \sim \Mult\left(\pi_{t}\right)$, where $\pi_{t}$
are topic probabilities gradually evolving along the simplex.

## Dynamic Topic Models {#sec:dtm}

LDA is an elementary count data modeling technique, and it has been adapted to a
variety of situations with more specific structure.
